...
public class SinglePositioning {
    ...
    public static void Get_SPP_POS(Epoch epoch, CMatrix XYZ,boolean use_PRC)
    {
        ...
        for (int i = 0; i < epoch.sat.size(); i++)
        {
            ...
            //下面计算观测值矩阵L
            //L观测值=P伪距观测值-P0站星距+e误差项(-e基站误差改正项)

            //计算卫星到基站距离,计算部分误差传递系数
            double Length = calculate_ErrorTransferCoefficient(XYZ, BL, sat_index, tempSat);

            //计算观测值，进行差分改正
            double measured_value = calculate_MeasuredValue(XYZ, tempSat, Length,0)+tempSat.PRC*(use_PRC?1:0);

            BL.p[sat_index][3] = measured_value;
    ...
    private static double calculate_MeasuredValue(CMatrix XYZ, Sat tempSat, double length, double error_tropDelay) {
        ...
        //计算观测值（测量值）  L观测值=P伪距观测值-P0站星距+e误差项(-e基站误差改正项)
        return -(length - tempSat.pseudo + total_error);
    }

    private static double calculate_ErrorTransferCoefficient(CMatrix XYZ, CMatrix BL, int sat_index, Sat tempSat) {
        ...
        double partialX = (tempSat.POS_X - XYZ.p[0][0]) / Length;
        double partialY = (tempSat.POS_Y - XYZ.p[1][0]) / Length;
        double partialZ = (tempSat.POS_Z - XYZ.p[2][0]) / Length;

        BL.p[sat_index][0] = -partialX;
        BL.p[sat_index][1] = -partialY;
        BL.p[sat_index][2] = -partialZ;
        return Length;
    }
    ...
    public static void Get_SPP_POS_Ele(Epoch epoch, CMatrix XYZ,boolean use_PRC)
    {
        ...
        for (int i = 0; i < epoch.sat.size(); i++)
        {
            ...
            //误差计算
            double error_left = calculate_MeasuredValue(XYZ, tempSat, Length,tempSat.Trop_Delay)+tempSat.PRC*(use_PRC?1:0);
...
//            Pos.p[2][0] = init_Z;
     ...
     */
    public static void Calc_Eph_GCEJ(double GPSTIME, double rclk, Sat sat, double[] XYZ, NavigationMessageRaw EpochNG)
    {
        switch(sat.sattype){
            case GPS:
                break;
